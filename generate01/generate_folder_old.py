...
def get_unique_top_methods(df: pd.DataFrame) -> List[str]:
    """
    Select the top 6 method IDs based purely on the 'sum' column. 
    ...
    Returns:
        List[str]: A list of up to 6 unique binning method IDs. 
    ...
    # 2. Select the top 6 method IDs
    # 提取前 6 个独特的方法 ID
    final_methods = df_sorted.head(6)['Binning_method'].tolist()
...
def process_selected_binning_methods(b_val: str):
    ...
    Workflow:
      ...
      2. Select best method IDs (Top 6 by sum, with the first one being the 'best')
    ...
    estimate_file_path = os.path.expanduser(f"{b_val}/estimate_res.txt")
    source_base_dir = os.path.expanduser(f"{b_val}/")
    ...
    # --- Step 2: Select IDs using the independent rules (Top 6 by sum) ---
    # 现在提取前 6 个方法
    ...
    # 打印实际选择的数量（最多 6 个）
    print(f"INFO: Selected {len(final_methods)} methods (max 6): {final_methods}", file=sys.stderr)
