...
def get_kmer_coverage(data_path: str, n_views: int = 2, kmer_model_path: str = 'empty',
    ...
    """
    namelist = pd.read_csv(data_path +'/aug/' 'aug0_datacoverage_mean.tsv', sep='\t', usecols=range(1)).values[:, 0]
    
    # #### tag
    # with open('/home/lyt/mnt/mydev/lyt/ArchLinkNew/datas/bam/contig_names_1000_clean.txt','r') as fp:
    #     lines = [line.strip() for line in fp]
        
    # new_name_list = [i for i in namelist if i in lines]
    # namelist = new_name_list
    #####
    
    mapObj = dict(zip(namelist, range(len(namelist))))
    
    for view in range(n_views):
        cov_file = data_path + 'aug/aug' + str(view) + '_datacoverage_mean.tsv'
        # print(cov_file)
        if not nokmer:
            com_file = data_path + 'aug/aug' + str(view) + '/kmer_4_f0.csv'
        ...
        if addvars:
            vars_file = data_path+'aug/'+'aug'+str(view)+'_datacoverage_var.tsv'
