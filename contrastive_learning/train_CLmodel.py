...
def train_CLmodel(logger, args):
    ...
    contig_file = args.data + '/aug/aug0/sequences_aug0.fasta'
    ...
    if args.notuseaug0:
        ...
        train_dataset = torch.utils.data.TensorDataset(*[dataset[i][np.array(length_weight) >= args.contig_len]                                                     for i in range(args.n_views)])
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=args.cl_batch_size, shuffle=True,
        num_workers=args.workers, pin_memory=True, drop_last=True)

    # print(len(train_dataset))
    # Set embedder model.
    if not args.add_model_for_coverage:
        ps = [args.cl_dropout_value] * (args.cl_n_layer - 1)
        actn = nn.LeakyReLU()

        emb_szs_list = [args.emb_szs] * args.cl_n_layer

        model = EmbeddingNet(
            in_sz=len(dataset[0][0]),
            out_sz=args.cl_out_dim,
        ...
        optimizer = torch.optim.AdamW(model.parameters(), args.cl_lr, weight_decay=args.cl_weight_decay)

        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.cl_epochs, eta_min=0,
                                                               last_epoch=-1)
        ...
        if args.pretrain_kmer_model_path !='no':
            ...
            input_size = args.cl_out_dim_forcov + 128
            print('cov_dim:\t' + str(cov_dim) + '\n')

            emb_szs_list = [args.emb_szs_forcov] * args.cl_n_layer_forcov

            cov_model = EmbeddingNet(
                in_sz=cov_dim,
                out_sz=args.cl_out_dim_forcov,
                emb_szs=emb_szs_list,
                ps=[args.cl_dropout_value] * (args.cl_n_layer_forcov - 1),
                use_bn=True,
                actn=nn.LeakyReLU(),
            )
            ...
            if args.finetunepretrainmodel:
                ...
                optimizer = torch.optim.AdamW([{'params': params_lx},
                                               {'params': model.pretrained_model.parameters(),
                                                'lr': args.cl_lr * args.finetunelr_ratio}],
                                              lr=args.cl_lr, weight_decay=args.cl_weight_decay)
            ...
            if args.kmer_model_path == 'empty':
                ...
                input_size = args.cl_out_dim_forcov + 136
            else:
            ...
            if args.pretrain_coveragemodel:
                ...
                optimizer = torch.optim.AdamW(cov_model.parameters(), args.cl_lr, weight_decay=args.cl_weight_decay)
