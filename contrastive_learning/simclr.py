...
class SimCLR(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # --- 稳定版日志配置 ---
        # 移除了双重文件逻辑，仅保留标准 training.log
        log_file = os.path.join(self.args.output_path, 'training.log')
        logging.basicConfig(
            level=logging.DEBUG,
    ...
    def info_nce_loss(self, features):
        ...
        """
        labels = torch.cat([torch.arange(self.args.cl_batch_size) for i in range(self.args.n_views)], dim=0)
        ...
        logits = logits / self.args.cl_temperature
    ...
    def train(self, train_loader, data, namelist):
        ...
        earlystop_epoch=0
        logging.info(f"Start SimCLR training for {self.args.cl_epochs} epochs.")
        print(self.args.cl_epochs)
        for epoch_counter in range(self.args.cl_epochs):
        ...
        checkpoint_name = 'checkpoint_{:04d}.pth.tar'.format(self.args.cl_epochs)
        save_checkpoint({
            'epoch': self.args.cl_epochs,
    ...
    def train_addpretrain(self, train_loader, data, namelist):
        ...
        with torch.no_grad():
            ...
            embeddings_df.to_csv(outfile, sep='\t', header=True)
            logging.info(f"Embeddings saved to {self.args.output_path}")
