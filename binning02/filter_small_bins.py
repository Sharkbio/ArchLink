...
def gen_bins(fastafile: str, resultfile: str, outputdir: str) -> None:
    ...
    with open(resultfile, "r") as f:
        for line in f:
            cluster_name, contig_name = line.strip().split('\t')
            try:
...
def filter_small_bins(logger, fastafile: str, resultfile: str, args, minbinsize: int = 200000) -> None:
    ...
    with open(resultfile, "r") as f:
        for line in f:
            # 交换了 contig_name 和 cluster_name 的位置
            cluster_name,contig_name = line.strip().split('\t')
            try:
                dic[cluster_name].append(contig_name)
    ...
    gen_bins(fastafile, outputdir, args.output_path + '/archlink_res_bins')
    shutil.copyfile(outputdir, args.output_path + '/archlink_res.tsv')
    # shutil.copy2(outputdir, args.output_path + '/archlink_res.tsv')
